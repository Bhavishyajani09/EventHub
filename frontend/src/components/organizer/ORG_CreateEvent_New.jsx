...
const CreateEvent = ({ isDark }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className={`text-2xl sm:text-3xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Create New Event</h1>
        <p className={`mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Fill in the details to create your event</p>
      </div>

      {/* Form */}
      <form onSubmit={handleSubmit} className={`rounded-lg shadow-md p-4 sm:p-6 space-y-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
        {/* Image Upload */}
        <div>
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 sm:space-x-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 w-full sm:w-auto"
          >
